 ...
 * @author Beyza Yaman
 * 
 * This metric calculates the number of valid Number of geo:hasgeometry properties...
 * Based on:
 *
 ...
 * 
 */
public class geohasGeometryPropertyCheck extends AbstractQualityMetric {

	private final Resource METRIC_URI = DQM.GeoHasGeometryCompletenessMetric;

	private static Logger logger = LoggerFactory.getLogger(geohasGeometryPropertyCheck.class);
...
//	protected Set<SerialisableModel> problemList =  MapDbFactory.createHashSet(mapDb, UUID.randomUUID().toString());
	...
	private boolean missingHasGeometryProperty =true;
	...
	public void compute(Quad quad) {
		...
		String lgdURI= "http://geovocab.org/geometry#geometry";
		String osiURI= "http://www.opengis.net/ont/geosparql#hasGeometry";
	...
	private void checkStandard(Node object, Node predicate, Node subject) {
		...
		if ((predicate.getURI().equals(GEOSPARQL.hasGeometryProperty.getURI()))) {
			propertyCount++;
			missingHasGeometryProperty =false;
		}
	}
	...
	public double metricValue() {
		...
		if(missingHasGeometryProperty==false) {
			for(Map.Entry<String, String> entry: resourceMap.entrySet()){
				if(entry.getKey().isEmpty()){
					createProblemQuad(entry.getValue().toString(),DQMPROB.MissingHasGeometryProperty);
				}}
			metricValue = ((double) this.propertyCount / this.geometryCount);
		}

		statsLogger.info("Dataset: {}; Number of Missing geo:hasGeometry Properties: {}, Number of total instances: {}, Metric Value: {}",this.getDatasetURI(),this.propertyCount, this.geometryCount, metricValue);
		return metricValue;
	}
